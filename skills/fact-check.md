...
- `references/foldable-display-supplier-history.md` — Display supplier chain for OPPO Find N series (N1–N6): Samsung vs BOE per generation, N3 switch event, key sources, OLED defect mechanism references
- `references/used-foldable-evaluation.md` — OLED defect diagnosis for buying used foldables: bright spot vs black spot, manufacturing vs pressure damage, UTG specs, ISO standards, risk assessment framework
- `references/root-matrix.md` — Bootloader unlock status by brand and OS version (XDA + DroidWin verified). Updated 2026.06 with full ARB/熔断 device list and EDL exploit patch status for Find N3
...
### 常见陷阱
- **verify_claims 工具不可靠**：该工具经常返回全部"查不到"（即使 web 搜索能轻松找到答案）。**不要依赖它**——当 verify_claims 返回全 ❓ 时，直接用手动 web_search/mcp_anysearch 逐条验证。它是便利工具但不应作为唯一验证手段。
- **Gitnux 数据需交叉验证**：Gitnux 的聚合数据（如 "1,900+ albergue"）可能被官方来源反驳（INE 仅 1,131）。优先用 INE、Xunta、Pilgrim's Office 等一手官方数据源。
...
- ⚠️ **技术实现推断陷阱（新增 2026.06.27）**：解释营销/技术声明时，不要把「合理的推断」当成事实。来源说「simulates XCD 30 lens bokeh」，不等于「AI trained on real XCD lens photos」——推断的技术手段（AI/传统图像处理）、涉及的镜头型号（我只查了 30/65/90V 三个镜头，但编造了第 4 个 XCD 80mm）、以及光学特征描述（"旋转焦外"）在没有来源明确支持时都是编的。**规则**：技术实现细节（how）必须来源明确说了才算，不要说「本质上是 XX」把猜测当结论。
